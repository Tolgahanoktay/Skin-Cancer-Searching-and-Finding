...
#map path içindeki her bir satiri fonksiyon yap anlamına gelmekte
plt.imshow(skin_df["image"][0])

skin_df["dx_idx"] = pd.Categorical(skin_df["dx"]).codes
skin_df.to_pickle("skin_df.pkl")
# %% load pkl
# #yukarida iki satirda kanser turleri olan dx leri int'e cevirdik ve indekslendirdik ve pickle formatinda sakladik
skin_df = pd.read_pickle("skin_df.pkl")

# %% stardardization
#standardization verileri karsilastirmak ve veri tipini yakalamak icin kullanilir
...
#3 tane renk katmanı var ve bunlar variable da belli 75,100,3
num_classes = 7

model = Sequential()
#sirali hale getirdik turleri
model.add(Conv2D(32, kernel_size = (3,3), activation = "relu", padding = "Same", input_shape = input_shape))
#32 tane noron kullanıyoruz kernel size yani filtre boyutu 3,3 activation ise fonksiyon,padding same ise veri kaybının onune gecmek icin kullaniliyor 
model.add(Conv2D(32, kernel_size = (3,3), activation = "relu", padding = "Same"))
#ilk ekledigimiz kisimda inputshape var cunku baslangicta onun bir inputu yok fakat sonrakiler sira ile output'u input olarak kullanacak
model.add(MaxPool2D(pool_size = (2,2)))
model.add(Dropout(0.25))
#her dongude noron sayisinin yukaridaki oran kadarini sondurup kullanmiyoruz bu ezberin onune gecip ogrenmeyi sagliyor

model.add(Conv2D(64, kernel_size = (3,3), activation = "relu", padding = "Same"))
model.add(Conv2D(64, kernel_size = (3,3), activation = "relu", padding = "Same"))
model.add(MaxPool2D(pool_size = (2,2)))
model.add(Dropout(0.5))
#buraya kadar olan kisim inputlayerlarimiz

#outupu layerlari yaziyoruz asagida siniflandirma icin ama siniflandirmaya gecmeden her zaman flatten yapmaliyiz
model.add(Flatten())
model.add(Dense(128,activation="relu"))
model.add(Dropout(0.5))
model.add(Dense(num_classes,activation="softmax"))
"""softmax class yada turu 2 den fazla olan class icin kullanilir bizim 7 tane 
kanser turummuz var cikan sonucu da siniflandirmamiz icin noron sayimizi class 
sayimiza bolmeliyiz ozet yaptigimiz kisim ise summary"""
model.summary()

optimizer = Adam(lr = 0.0001)
model.compile(optimizer = optimizer, loss = "categorical_crossentropy", metrics = ["accuracy"])

epochs = 5 #verilerimizin kac kez tekrar edilicegini belirttik
batch_size = 100
"""burda da istersek elimizdeki veri sayisini da girebilirdik fakat 
hepsini birden epohlayamayiz bellek hatasi aliriz bu yuzden 25'erli sekilde ele aliyoruz 25*5 """
...
def openImage():
    ...
    count += 1 #kullanmamizin sebebi yalnizca 1 resim sectirmek
    if count != 1:
        ...
        img_jpg = img_name.split("/")[-1].split(".")[0]#imagejpg resmin id sinden olusmakta
        # image label
...
def classification():
    if img_name != "" and models.get() != "":
        ...
        # standardize ettik ve en yuksek indexi bulup gorsellestirdik
